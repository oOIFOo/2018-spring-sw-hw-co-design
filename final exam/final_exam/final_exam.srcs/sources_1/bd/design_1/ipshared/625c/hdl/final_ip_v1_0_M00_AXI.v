...
`timescale 1 ns / 1 ps
	...
	(
		...
		input  wire [C_M_AXI_DATA_WIDTH-1:0] the_second_max,
		input  wire [C_M_AXI_DATA_WIDTH-1:0] the_second_min,
		input  wire [C_M_AXI_DATA_WIDTH-1:0] dst_addr,
		input  wire part2_done,
		output reg  part3_done,
  ...
  /* Write Data Generator */
	...
	// Next address after ARREADY indicates previous address acceptance  
	  ...
	  begin                                                              
	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)                                          
	      begin                                                          
	        axi_araddr <= 0;  // you should ignore it, because you just write data to memory.                                         
	      end                                                            
	    else if (M_AXI_ARREADY && axi_arvalid)                           
	      begin                                                          
	...
	// Read a data burst into the buffer
	  ...
	  begin
	    ...
	    if (rnext)
	        if(read_index == 0) buffer[read_index] <= the_second_max;
	        else if(read_index == 1) buffer[read_index] <= the_second_min;
	  end
