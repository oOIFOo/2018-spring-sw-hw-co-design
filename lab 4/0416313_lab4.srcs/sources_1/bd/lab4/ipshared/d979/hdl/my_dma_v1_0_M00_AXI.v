...
`timescale 1 ns / 1 ps
    ...
    (* mark_debug = "true" *) input wire  [C_M_AXI_DATA_WIDTH-1:0] len_copy,
    output reg                           hw_done,
		// User ports ends
	...
	parameter [1:0]
	  IDLE = 2'b00, // This state initiates AXI4 transaction
		...
		INIT_WRITE = 2'b10, // This state initializes write transaction,
			...
			// changes state to COPY_DONE
		COPY_DONE  = 2'b11; // This state issues the status of comparison
	...
	(* mark_debug = "true" *) reg [C_TRANSACTIONS_NUM : 0] 	write_index;
    ...
    (* mark_debug = "true" *) reg     [9:0] total_length;
    (* mark_debug = "true" *) reg     [5:0] burst_length;
	...
	always @(posedge M_AXI_ACLK)
	  begin
	    ...
	    if (M_AXI_ARESETN == 0 )
	      ...
	      begin
	        init_txn_ff <= hw_active | (mst_exec_state == INIT_WRITE);
	...
	// Next address after AWREADY indicates previous address acceptance    
	  ...
	  begin                                                                
	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)                                            
	      ...
	      begin                                                            
	        axi_awaddr <= axi_awaddr + burst_length*4;                   
	      end                                                              
	    else                                                               
	...
	/* Burst length counter. Uses extra counter register bit to indicate terminal       
	 count to reduce decode logic */                                                    
	  always @(posedge M_AXI_ACLK)                                                      
	  begin                                                                             
	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1 || start_single_burst_write == 1'b1 || mst_exec_state == INIT_READ)    
	      begin                                                                         
	        write_index <= 0;                                                           
	      end                                                                           
  ...
  /* Write Data Generator */
	...
	// Next address after ARREADY indicates previous address acceptance  
	  ...
	  begin                                                              
	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1)                                          
	      ...
	      begin                                                          
	        axi_araddr <= axi_araddr + burst_length*4;                 
	      end                                                            
	    else                                                             
	...
	// terminal count to reduce decode logic                                
	  always @(posedge M_AXI_ACLK)                                          
	  begin                                                                 
	    if (M_AXI_ARESETN == 0 || init_txn_pulse == 1'b1 || start_single_burst_read || mst_exec_state == INIT_WRITE)                  
	      begin                                                             
	        read_index <= 0;                                                
	      end                                                               
	...
	integer i;
	  ...
	  begin
	    ...
	    if(mst_exec_state == IDLE) begin
	        for(i = 0; i < C_M_AXI_DATA_WIDTH; i = i+1) begin
	           buffer[i] <= 0;
	        end
	    end
	    else if (rnext)
	...
	//Flag any read response errors                                         
	 ...
	 // committed.                                                                                              
	  ...
	  begin                                                                                                     
	    ...
	    else if (M_AXI_BVALID && (write_index == burst_length-1) && axi_bready)                          
	      writes_done <= 1'b1;                                                                                  
	    else                                                                                                    
	...
	// Add user logic here
      ...
      always @(posedge M_AXI_ACLK) begin
        if (M_AXI_ARESETN == 0 || mst_exec_state == COPY_DONE) begin                                                                                 
            flag <= 0;
            total_length <= 0;
            burst_length <= 0;
        end
        else if(mst_exec_state == IDLE) begin
            if(len_copy >= 4) total_length <= len_copy / 4 + 1;
            else total_length <= 1;
        end
        else if(mst_exec_state == INIT_READ && ~flag) begin
            if(total_length >= 16) begin
                burst_length <= 16;
                total_length <= total_length - 16;
            end
            else if(total_length >= 8) begin
                burst_length <= 8;
                total_length <= total_length - 8;
            end
            else if(total_length >= 4) begin
                burst_length <= 4;
                total_length <= total_length - 4;
            end
            else if(total_length >= 2) begin
                burst_length <= 2;
                total_length <= total_length - 2;
            end
            else if(total_length == 1)begin
                burst_length <= 1;
        ...
        else if(mst_exec_state == INIT_WRITE) begin
            flag <= 0;
        end
      end
	// User logic ends
